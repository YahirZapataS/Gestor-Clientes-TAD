...
async function fetchClientHistory(clientId) {
    ...
    if (!creditSnap.empty) {
        creditSnap.forEach(doc => {
            const data = doc.data();
            const fecha = data.date instanceof Timestamp ? data.date.toDate() : new Date();
            const items = data.items || [];

            const productosTexto = items.map(p =>
    ...
    if (currentClientHistory.length === 0) {
        ...
        currentClientHistory.forEach(entry => {
            const fecha = entry.fecha.toLocaleString("es-MX", { dateStyle: "medium", timeStyle: "short" });
...
function exportClientHistoryToPDF() {
    ...
    setTimeout(() => {
        ...
        const clientId = document.getElementById("clientIdHeader")?.textContent || "-";
...
async function exportGeneralReportPDF() {
    ...
    for (const clientDoc of clientsSnap.docs) {
        ...
        const [creditsSnap, paymentsSnap] = await Promise.all([
            getDocs(query(collection(db, "credits"), where("clientId", "==", clientId))),
